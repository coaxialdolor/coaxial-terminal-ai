...
def parse_arguments():
    ...
    parser.add_argument("--project-path", type=str,
                        help="Specify project path directly (bypass interactive selection)")
    parser.add_argument("--version", type=str,
                        help="Specify new version directly (bypass interactive selection)")
    ...
    special_ops = [args.no_upload, args.only_upload, args.revert, args.test_mode, args.create_config]
    if sum(special_ops) > 1:
...
def main():
    ...
    try:
        ...
        if args.project_path:
            ...
            print_color(f"Using specified project path: {project_path}", COLOR_GREEN)
        else:
        ...
        if args.version:
            ...
            print_color(f"Using specified version: {new_version}", COLOR_GREEN)
        else:
